...
if (isset($_SESSION['account-type']) && ((isset($_GET['id']) && $_GET['id'] > 0) || (isset($_POST['id']) && $_POST['id'] > 0))) {
    ...
    {   
        if('NOHEADER' == $img){
            ...
            $html .= '<div style="margin-top: 60px">
                        <img src="https://beta.nogd.no/images/logo-nogd-rapport.jpg" style="width: 170px; height: 60px; float: left; margin-left: 50px;margin-bottom:20px;" /><br>
    ...
    function free_text4($report, $trans, $index)
    {
        ...
        if ($report['free_text4']['enabled']) {
            ...
            $catChart = $_POST['catChart'];
            $html .= '<table style="width:100%;"><tr><td align="center"><img src="'.$catChart.'" style="margin-top:100px;width: 600px; height: 600px;" /></td></tr></table>';

            $html .= '<div style="margin-left: 25px; margin-right: 20px;margin-top:90px;">'.htmlspecialchars_decode($report['free_text4']['text']).'</div>';
    ...
    function free_text5($report, $trans, $index)
    {
        ...
        if ($report['free_text5']['enabled']) {
            ...
            $radarChart = $_POST['radarChart'];

            if(isset($radarChart) && !empty($radarChart)){
                $html .= '<table style="width:100%;"><tr><td align="center"><img src="'.$radarChart.'" style="margin-top:100px;width: 600px; height: 600px;" /></td></tr></table>';
            }
    ...
    if ($report) {
        ...
        if ($report['free_text9']['enabled']) {
            $mpdf->AddPage('P', '', '', '', '', 20, 10, 14, 10, 8, 10);
            $mpdf->SetHTMLHeader();
            $page_header = page_header('NOHEADER');
